...
** Meow Mode
Yet another modal editing for Emacs with QWERTY layout.
...
*** Odin mode
Support for the Odin programming language with modern Tree-sitter syntax highlighting, indentation, and LSP integration (OLS + Eglot).
#+begin_src emacs-lisp
...
*** Go mode
Support for Go with proper tab formatting and LSP integration (gopls + Eglot).
#+begin_src emacs-lisp
    (use-package go-mode
      :mode "\\.go\\'"
      :hook ((go-mode . (lambda ()
